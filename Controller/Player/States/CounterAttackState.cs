...
public class CounterAttackState : GenealState
{
   ...
   [Header("Animation Frame Variables")]
    ...
    //일단 빠르게 카운터 어택을 했을경우 move로 갔을때 reEnter 딜레이가 존재함... (?) 
    //머가문제지.
    ...
    public override void Exit(PlayerStateController stateController)
    {
        ...
        //CommonUIManager.Instance.ExcuteGlobalNotifer("CounterExit ");  //테스트
    ...
    //일단 타입 슬로우를 안하고 바로 공격했을 경우 멈춤..
    //조금 있다가 공격할 경우 멈추지 않음. 머가문제지.
    ...
    private IEnumerator CounterAttackProcess()
    {
        ...
        canParry = false;
        TimeManager.Instance.TimeReset();
        //CommonUIManager.Instance.ExcuteGlobalSimpleNotifer("카운터 어택!");  //테스트
        ...
        // 공격 애님이 끝날을때 패링성공했다면 그냥 종료
        // 공격 애님이 끝났을때 패링 성공을 안헀다면 move
        if (!excuteParrying)
        {
            Debug.Log("종료1");
            controller.ChangeState(controller.moveStateHash,0);
