...
public class AttackState : GenealState
{
    ...
    {
        //if (isStart)   // 테스트용
       //     timer += Time.deltaTime;    // 테스트용
       //if (isinput)
       //    inputTimer += Time.deltaTime;

       // HandleInputAttackType(); 으로 정리 updateAction부분은 깔끔하게 해두기.
    ...
    // 코루틴 데미지
    // 코루틴 End 검사. 
    // 두가지 코루틴으로 검사 실행함.  end시까지 can combo Input이 가능함.
    // 중요. canComboinput을 공격 직후로 잡음. 그리하여 바로 콤보를 할수있게함. 
    // 콤보 입력없으면 End 검사로 인해 자동 move 상태로.
    // 콤보 입력시 바로 공격을함. (canComboInput으로 타이밍을 정해서, 공격 끝나는 지점에서 바로 input을 받을수있기때문에)
    /// </summary>
